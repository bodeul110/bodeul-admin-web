...
test("승인은 현재 자격 증빙의 세대와 digest를 snapshot으로 고정한다", () => {
  ...
  assert.deepEqual(
    ...
    [
      {documentKey: "license", generation: "100", digest: "4".repeat(64), submissionRevision: REVISION},
    ],
  );
});
...
test("조회 후 revision 또는 문서 포인터가 바뀐 race를 거부한다", () => {
  ...
  assert.throws(() => validateManagerReviewTransition({
    ...validInput(),
    currentStoragePathDigests: {...validInput().currentStoragePathDigests, license: "9".repeat(64)},
  }), (error: unknown) => (error as {code?: string}).code === "manager_document_evidence_stale");
});

test("승인은 현재 canonical 키와 증거가 각각 정확히 하나일 때만 허용한다", () => {
  assert.throws(() => validateManagerReviewTransition({
    ...validInput(),
    documentEvidence: [],
  }), (error: unknown) => (error as {code?: string}).code === "manager_document_evidence_stale");
  ...
  assert.throws(() => validateManagerReviewTransition({
    ...
    currentStoragePathDigests: {
      license: "1".repeat(64),
      nursingLicense: "2".repeat(64),
    },
  }), (error: unknown) => (error as {code?: string}).code === "manager_document_evidence_stale");
  assert.throws(() => validateManagerReviewTransition({
    ...validInput(),
    documentEvidence: [{...evidence()[0]!, documentKey: "nursingLicense"}],
  }), (error: unknown) => (error as {code?: string}).code === "manager_document_evidence_stale");
});
